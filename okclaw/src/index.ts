...
async function processGroupMessages(chatJid: string): Promise<boolean> {
  ...
  // Track the active conversationId for this group
  if (conversationId) {
    queue.setActiveConversationId(chatJid, conversationId);
  }
    ...
    try {
      ...
      if (conversation && conversation.name === '新对话') {
        ...
        const currentSessionId =
          sessions[sessionKey] || agentResult.newSessionId;
        ...
        const titleResult = await runAgentDirect(
          ...
          {
            prompt:
              '请为我们的对话生成一个2-6个汉字的简短标题，只输出标题本身，不要任何标点符号、引号或其他内容。',
        ...
        if (
          ...
          logger.info(
            { convId: conversationId, title: cleanTitle },
...
async function main(): Promise<void> {
  ...
  const channelOpts = {
    ...
    registerGroup,
    // Get active conversationId for a workspace (used by Web IM to restore typing state on refresh)
