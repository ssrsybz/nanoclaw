...
export async function parseFile(
  ...
  if (buffer.length > MAX_FILE_SIZE) {
    throw new Error(
      `File size (${(buffer.length / 1024 / 1024).toFixed(1)}MB) exceeds maximum (${MAX_FILE_SIZE / 1024 / 1024}MB)`,
    );
  }
  ...
  try {
    ...
    if (
      ...
      (err.message.includes('Unsupported') || err.message.includes('exceeds'))
    ) {
      throw err;
    }
