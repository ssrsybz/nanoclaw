...
interface PendingQuestion {
  ...
  resolve: (
    ...
    annotations?: AskUserQuestionResponse['annotations'],
  ) => void;
...
export async function waitForQuestionResponse(
  ...
  return new Promise((resolve, reject) => {
    ...
    logger.info(
      { conversationId, toolUseId, questionCount: questions.length },
...
export function handleQuestionResponse(
  ...
  if (!pending) {
    logger.warn(
      ...
      'No pending question for response',
    );
    return false;
  }
  ...
  if (pending.toolUseId !== response.toolUseId) {
    logger.warn(
      {
        conversationId: response.conversationId,
        expected: pending.toolUseId,
        got: response.toolUseId,
      },
 ...
 */
export function writeQuestionResponseViaIPC(
  response: AskUserQuestionResponse,
): void {
  ensureIpcDir();
  const filePath = path.join(
    QUESTION_IPC_DIR,
    `${response.conversationId}-${response.toolUseId}.json`,
  );
