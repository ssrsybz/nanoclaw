...
nameZh: 前端工程师
description: Pro frontend engineering discipline. Enforces build-test-verify workflow for every web project. Never declare done until the site is built, tested, responsive, accessible, and visually verified in a real browser.
...
When the project is ready for deployment, use the appropriate deployment method for the project's setup. If Vercel is configured, use `/add-vercel` skill to set it up, then deploy via:

```bash
vercel deploy --prod
```

For other deployment targets, follow the project's existing deployment configuration.
