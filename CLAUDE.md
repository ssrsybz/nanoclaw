...
基于 Vite + React + TypeScript 的 SPA，位于 `web/`。主要功能：
- 多工作空间支持，含对话管理
...
```bash
cd web && npm run dev    # 开发服务器 :5173
cd web && npm run build  # 构建到 web/dist/
```

## 技能

NanoClaw 有四种技能类型。详见 [CONTRIBUTING.md](CONTRIBUTING.md) 了解完整分类和规范。
...
| `/update-nanoclaw` | 将上游 NanoClaw 更新同步到自定义安装 |
...
- `data/ipc/` — 跨进程通信的消息队列
- `store/nanoclaw.db` — SQLite 数据库（群组、消息、工作空间、对话、任务）
