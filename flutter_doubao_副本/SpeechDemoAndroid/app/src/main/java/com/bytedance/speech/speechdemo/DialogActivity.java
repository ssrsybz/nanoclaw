...
public class DialogActivity extends BaseActivity implements SpeechEngine.SpeechListener, LifecycleObserver {
    ...
    private Button mChatTextQueryBtn;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mChatTextQueryBtn = findViewById(R.id.chat_text_query);
        setButton(mChatTextQueryBtn, false);
        mChatTextQueryBtn.setOnClickListener(v -> chatTextQuery());

        mHelloEt = findViewById(R.id.hello_edit_text);
        mChatTextEt = findViewById(R.id.chat_text_edit_text);
    ...
    private void configInitParams() {
        ...
        //【可选配置】是否开启播放器，默认开启
        mSpeechEngine.setOptionBoolean(SpeechEngineDefines.PARAMS_KEY_DIALOG_ENABLE_PLAYER_BOOL, mSettings.getBoolean(R.string.config_dialog_enable_player));
        ...
        mSpeechEngine.setOptionBoolean(SpeechEngineDefines.PARAMS_KEY_DIALOG_ENABLE_DECODER_AUDIO_CALLBACK_BOOL, false);
        //【可选配置】自定义请求Header
    ...
    private void sayHello() {
        ...
        if (ret != SpeechEngineDefines.ERR_NO_ERROR) {
            String tips = "Send directive say hello failed, " + ret;
            Log.e(SpeechDemoDefines.TAG, tips);
            mEngineStatusTv.setText(tips);
            stopEngine();
        } else {
    ...
    private void chatTextQuery() {
        // Directive：发送chat_text_query指令以发送Query文本。
        String queryText = String.format("{\"content\": \"%s\"}", mChatTextEt.getText().toString());
        int ret = mSpeechEngine.sendDirective(SpeechEngineDefines.DIRECTIVE_EVENT_CHAT_TEXT_QUERY, queryText);
        if (ret != SpeechEngineDefines.ERR_NO_ERROR) {
            String tips = "Send directive chat text query failed, " + ret;
            Log.e(SpeechDemoDefines.TAG, tips);
            mEngineStatusTv.setText(tips);
            stopEngine();
        } else {
            showChatTextQueryMessage(mChatTextEt.getText().toString());
        }
    }
    ...
    public void speechEngineStarted(String sessionId) {
        ...
        runOnUiThread(() -> {
            ...
            setButton(mChatTextQueryBtn, true);
        });
    }
    ...
    private void showChatTextQueryMessage(String data) {
        runOnUiThread(() -> {
            confirmAssistantMessage();
            mDialogMessages.add(new Message(Role.USER, data, true));
            updateMessageUI();
        });
    }
