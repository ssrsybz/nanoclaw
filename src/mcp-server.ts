...
export interface McpCallbacks {
  sendMessage: (chatJid: string, text: string, sender?: string) => Promise<void>;
  scheduleTask: (task: {
    id: string;
    prompt: string;
    script?: string;
  ...
  pauseTask: (taskId: string, groupFolder: string, isMain: boolean) => Promise<void>;
  resumeTask: (taskId: string, groupFolder: string, isMain: boolean) => Promise<void>;
  cancelTask: (taskId: string, groupFolder: string, isMain: boolean) => Promise<void>;
  updateTask: (taskId: string, updates: Record<string, string | undefined>, groupFolder: string, isMain: boolean) => Promise<void>;
  registerGroup: (jid: string, name: string, folder: string, trigger: string) => Promise<void>;
  listTasks: (groupFolder: string, isMain: boolean) => Promise<Array<{
    id: string;
    prompt: string;
  ...
  listAvailableGroups: (isMain: boolean) => Promise<Array<{
    jid: string;
    name: string;
 ...
 */
export function createMcpTools(context: McpContext, callbacks: McpCallbacks): McpTool[] {
  ...
  return [
    {
      ...
      handler: async (args) => {
        await callbacks.sendMessage(chatJid, args.text as string, args.sender as string | undefined);
...
• once: Local time WITHOUT "Z" suffix (e.g., "2026-02-01T15:30:00"). Do NOT use UTC/Z suffix.`,
      inputSchema: z.object({
        ...
        script: z.string().optional().describe('Optional bash script to run before waking the agent.'),
      }),
      handler: async (args) => {
        ...
        if (scheduleType === 'cron') {
          try {
            ...
            return {
              content: [{ type: 'text', text: `Invalid cron: "${scheduleValue}". Use format like "0 9 * * *" (daily 9am) or "*/5 * * * *" (every 5 min).` }],
              isError: true,
            };
          }
          ...
          if (isNaN(ms) || ms <= 0) {
            return {
              content: [{ type: 'text', text: `Invalid interval: "${scheduleValue}". Must be positive milliseconds (e.g., "300000" for 5 min).` }],
              isError: true,
            };
          }
          ...
          if (/[Zz]$/.test(scheduleValue) || /[+-]\d{2}:\d{2}$/.test(scheduleValue)) {
            return {
              content: [{ type: 'text', text: `Timestamp must be local time without timezone suffix. Got "${scheduleValue}" — use format like "2026-02-01T15:30:00".` }],
              isError: true,
            };
          }
          ...
          if (isNaN(date.getTime())) {
            return {
              content: [{ type: 'text', text: `Invalid timestamp: "${scheduleValue}". Use local time format like "2026-02-01T15:30:00".` }],
              isError: true,
            };
          }
        }
        ...
        return {
          content: [{ type: 'text', text: `Task ${taskId} scheduled: ${scheduleType} - ${scheduleValue}` }],
        };
      },
    },
    ...
    {
      name: 'update_task',
      description: 'Update an existing scheduled task. Only provided fields are changed; omitted fields stay the same.',
      inputSchema: z.object({
      ...
      handler: async (args) => {
        ...
        if (args.schedule_type !== undefined) updates.schedule_type = args.schedule_type as string;
        if (args.schedule_value !== undefined) updates.schedule_value = args.schedule_value as string;

        await callbacks.updateTask(args.task_id as string, updates, groupFolder, isMain);
