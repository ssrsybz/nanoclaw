...
• once: Local time WITHOUT "Z" suffix (e.g., "2026-02-01T15:30:00")`,
  {
    ...
    schedule_value: z.string().describe('Schedule value (format depends on type)'),
    context_mode: z.enum(['group', 'isolated']).default('group').describe('Context mode'),
    target_group_jid: z.string().optional().describe('(Main only) Target group JID'),
    script: z.string().optional().describe('Optional bash script to run before the agent'),
  },
  async (args) => {
    ...
    if (args.schedule_type === 'cron') {
      ...
      if (/[Zz]$/.test(args.schedule_value) || /[+-]\d{2}:\d{2}$/.test(args.schedule_value)) {
        return {
          content: [{ type: 'text' as const, text: `Timestamp must be local time without timezone suffix.` }],
          isError: true,
        };
      }
