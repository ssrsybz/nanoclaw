...
│  │    • mcp__okclaw__* (scheduler tools via IPC)                │    │
│  │                                                                │    │
│  └──────────────────────────────────────────────────────────────┘    │
│                                                                       │
└───────────────────────────────────────────────────────────────────────┘
...
│   ├── okclaw.log               # Host stdout
│   └── okclaw.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.okclaw.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'okclaw-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your okclaw installation
...
OKClaw uses a hierarchical memory system based on CLAUDE.md files.
...
OKClaw has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__okclaw__schedule_task]
        {
...
The `okclaw` MCP server is created dynamically per agent call with the current group's context.
...
OKClaw runs as a single macOS launchd service.

### Startup Sequence

When OKClaw starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned OKClaw containers from previous runs
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/okclaw.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/okclaw.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.okclaw.plist ~/Library/LaunchAgents/
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep okclaw` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; OKClaw auto-starts container runtime but may fail |
...
- `logs/okclaw.log` - stdout
- `logs/okclaw.error.log` - stderr
