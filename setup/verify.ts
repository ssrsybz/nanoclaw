...
export async function run(_args: string[]): Promise<void> {
  ...
  if (mgr === 'launchd') {
    try {
      ...
      if (output.includes('com.okclaw')) {
        // Check if it has a PID (actually running)
        const line = output.split('\n').find((l) => l.includes('com.okclaw'));
        if (line) {
    ...
    try {
      execSync(`${prefix} is-active okclaw`, { stdio: 'ignore' });
      service = 'running';
    } catch {
      try {
  ...
  if (
    fs.existsSync(
      path.join(homeDir, '.config', 'okclaw', 'mount-allowlist.json'),
    )
  ) {
