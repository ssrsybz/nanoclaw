...
<dict>
    ...
    <string>${projectRoot}/logs/okclaw.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/okclaw.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.okclaw');
  } catch {
...
/**
 * Kill any orphaned okclaw node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned okclaw processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/okclaw.service';
    ...
    unitPath = path.join(unitDir, 'okclaw.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=OKClaw Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/okclaw.log
StandardError=append:${projectRoot}/logs/okclaw.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned okclaw processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable okclaw`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start okclaw`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active okclaw`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-okclaw.sh');
  const pidFile = path.join(projectRoot, 'okclaw.pid');

  const lines = [
    '#!/bin/bash',
    '# start-okclaw.sh — Start OKClaw without systemd',
    ...
    '    echo "Stopping existing OKClaw (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting OKClaw..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/okclaw.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/okclaw.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "OKClaw started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/okclaw.log"`,
  ];
