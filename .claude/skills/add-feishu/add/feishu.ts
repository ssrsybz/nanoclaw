...
export class FeishuChannel implements Channel {
  ...
  private async handleMessage(data: any): Promise<void> {
    ...
    if (chatType === 'p2p') {
      // Private chat
      chatJid = `fs:p2p:${message.sender?.id || message.sender?.open_id || 'unknown'}`;
      chatName = message.sender?.name || 'Private Chat';
      isGroup = false;
      senderId = message.sender?.id || message.sender?.open_id || '';
      senderName = message.sender?.name || 'Unknown';
    } else if (chatType === 'group') {
      // Group chat
      chatJid = `fs:group:${message.chat_id}`;
      chatName = message.chat_name || `Group ${message.chat_id}`;
      isGroup = true;
      senderId = message.sender?.id || message.sender?.open_id || '';
      senderName = message.sender?.name || 'Unknown';
    } else {
    ...
    if (!group) {
      logger.debug({ chatJid }, 'Feishu chat not registered, skipping');
      return;
    }
