...
启动成功后访问 **http://localhost:3100** 即可使用 Web 界面。

## 注意事项

- **单进程架构**：前后端在同一个 Node.js 进程中运行，无需单独启动
- **端口占用**：确保 3100 端口未被其他程序占用
- **凭证配置**：LLM 凭证通过 `~/.claude/settings.json` 的 env 块配置，非 LLM 密钥（如飞书 Token）放项目 `.env`
- **安全提醒**：Agent 的 Bash 命令直接在宿主机执行，仅在可信环境中使用
- **停止服务**：终端按 `Ctrl+C` 或 `kill` 进程即可
...
OKClaw 提供相同的核心功能，但代码量小到你能完全理解：一个进程，几个文件。Claude Agent 直接在主进程中运行，无需容器。
...
1. 在 GitHub 上 Fork [qwibitai/okclaw](https://github.com/qwibitai/okclaw)
...
然后打开浏览器访问 `http://localhost:3100`，开始对话。
...
**小到能理解。** 一个进程，几个源文件，没有微服务。想了解整个 OKClaw 代码库？直接让 Claude Code 带你过一遍。

**为个人用户设计。** OKClaw 不是臃肿的框架，而是贴合每个用户需求的软件。你 Fork 一份，让 Claude Code 按你的需要修改。
...
OKClaw 不使用配置文件。直接告诉 Claude Code 你想要什么：
...
如果你想添加 Telegram 支持，不要创建把 Telegram 加进核心代码库的 PR。而是 Fork OKClaw，在分支上做代码改动，然后开 PR。我们会从你的 PR 创建 `skill/telegram` 分支，其他用户可以合并到自己的 Fork。
...
完整架构细节见[文档站](https://docs.okclaw.dev/concepts/architecture)。
...
可以。OKClaw 是纯 Node.js，在 macOS、Linux 和 Windows（WSL2）上都能运行。直接 `/setup` 即可。

**安全吗？**

Agent 直接在主进程运行，拥有完整的工具权限。建议仅在可信环境中使用，代码量小到你可以审计每一行。详见[安全文档](https://docs.okclaw.dev/concepts/security)。

**为什么不用配置文件？**

我们不想要配置膨胀。每个用户应该定制自己的 OKClaw，让代码做你想要的事，而不是配置一个通用系统。如果你偏好配置文件，可以让 Claude 加上。
...
问 Claude Code。"为什么定时任务没运行？" "最近的日志有什么？" "这条消息为什么没得到回复？" 这就是 OKClaw 的 AI 原生方式。
...
详见 [CHANGELOG.md](CHANGELOG.md) 或文档站的[完整发布历史](https://docs.okclaw.dev/changelog)。
